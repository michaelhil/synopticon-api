...
export interface UsageStatistics {
  ...
  pipelineUsage: Record<string, PipelineUsageStats>;
  contextHistorySize: number;
}

export interface UsageTracker {
  recordPipelineUsage: (pipelineType: string, specificContext?: string | null) => void;
  calculatePreloadPriority: (pipelineType: string) => number;
  getUsageStatistics: () => UsageStatistics;
  getUsageHistory: () => Map<string, UsageRecord[]>;
  getContextHistory: () => ContextHistoryEntry[];
}

export const createUsageTracker = (deviceDetector: DeviceDetector, config: PreloaderConfig): UsageTracker => {
  const usageHistory = new Map<string, UsageRecord[]>();
  const contextHistory: ContextHistoryEntry[] = [];

  /**
   * Load usage history from localStorage
   */
  const loadUsageHistory = (): void => {
    ...
    try {
      if (typeof localStorage !== 'undefined') {
        ...
        if (saved) {
          const parsed = JSON.parse(saved) as Record<string, UsageRecord[]>;
   ...
   */
  const calculatePreloadPriority = (pipelineType: string): number => {
    const usage = usageHistory.get(pipelineType) || [];
   ...
   */
  const getUsageStatistics = (): UsageStatistics => {
    const totalUsageRecords = Array.from(usageHistory.values())
      .reduce((sum, records) => sum + records.length, 0);

    const pipelineUsage: Record<string, PipelineUsageStats> = {};

...
export const createEmotionConfig = (config = {}) => ({
  ...
  prosodics: {
    ...
    sampleRate: config.sampleRate || 44100,
    pitchRange: config.pitchRange || [80, 400],
  ...
  classifier: {
    ...
    emotionMappingWeights: config.emotionMappingWeights || {
...
export const validateEmotionConfig = (config) => {
  const errors = [];
  ...
  if (config.prosodics) {
    const [minPitch, maxPitch] = config.prosodics.pitchRange || [80, 400];
...
export const updateEmotionConfig = (currentConfig, updates) => {

...
import { createRouteRegistry, type RouteDependencies } from './routes/index.js';
import { createWebSocketManager, type WebSocketDependencies } from './websocket/index.js';
import { createMiddlewareSystem } from './middleware/index.js';
import {
  type MiddlewareSystem,
  type MiddlewareConfig
} from './middleware/middleware-types.js';
import { createUnifiedConfigManager, type UnifiedConfigManager, type UnifiedConfig } from '../../core/configuration/unified-config-manager.js';
...
export const createFaceAnalysisServer = (initialConfig: Partial<UnifiedConfig> = {}): ServerInstance => {
  // Initialize unified configuration manager
  const configManager = createUnifiedConfigManager(initialConfig);
  const serverConfig = configManager.getServerConfig();
  const port = serverConfig.port;
  
  const orchestrator = createUnifiedOrchestrator({
    ...configManager.getOrchestrationConfig()
  });
  
  // Initialize memory optimizer  
  const memoryOptimizer = createMemoryOptimizer({
    memoryPressureThreshold: configManager.get('performance.memoryPressureThreshold') || 0.75
  });
  ...
  const getMediaStreamingAPI = async (): Promise<any> => {
    if (!mediaStreamingAPI) {
      ...
      const mediaConfig = configManager.getFeaturesConfig().mediaStreaming;
      mediaStreamingAPI = modules.createMediaStreamingAPI({
        httpHost: serverConfig.host,
        httpPort: port,
        websocketHost: configManager.get('distribution.adapters.websocket.host') || '0.0.0.0',
        websocketPort: configManager.get('distribution.adapters.websocket.port') || 8765,
        discoveryInterval: mediaConfig.discoveryInterval || 10000
      });
  ...
  const getDistributionSystem = async (): Promise<{
    ...
    if (!distributionSessionManager) {
      ...
      // Use unified configuration for distribution
      const distributionConfig = configManager.getDistributionConfig();
      distributionSessionManager = modules.createUniversalDistributor(distributionConfig);
      console.log('✅ Universal distribution system lazy loaded with unified config');
    }
    return { distributionSessionManager, configManager };
  };
  ...
  const createDistributionStream = async (streamConfig: any): Promise<DistributionStream> => {
    ...
    const { distributionSessionManager } = await getDistributionSystem();
  ...
  const wsConfig = configManager.get('distribution.adapters.websocket');
  const wsManager = createWebSocketManager({
    orchestrator,
    initializeEmotionPipeline,
    config: {
      maxWebSocketSessions: wsConfig.maxConnections || 100,
      websocketTimeout: configManager.get('distribution.defaultTimeout') || 300000,
  ...
  const middlewareConfig: MiddlewareConfig = {
    cors: {
      ...
      allowCredentials: serverConfig.cors.credentials,
      logRequests: configManager.getLoggingConfig().level === 'debug'
    },
    rateLimiting: {
    ...
    errorHandler: {
      enableDetailedErrors: configManager.getEnvironmentConfig().debug,
      enableErrorLogging: configManager.getLoggingConfig().enableConsole
    }
  };

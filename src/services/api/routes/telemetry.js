...
const simulatorInstances = new Map();
let telemetryStreams = new Map();
...
export const createTelemetryRoutes = ({
  ...
  const getSimulatorInstance = async (type, config = {}) => {
  ...
  routes.push(['GET', '/api/telemetry/simulators', async (request) => {
    try {
      ...
      return createErrorResponse('Failed to list simulators', 500, error);
    }
  }]);

  // POST /api/telemetry/connect - Connect to a simulator
  routes.push(['POST', '/api/telemetry/connect', async (request) => {
    try {
      ...
      return createErrorResponse('Connection error', 500, error);
    }
  }]);

  // DELETE /api/telemetry/disconnect/:type - Disconnect from simulator
  routes.push(['DELETE', '^/api/telemetry/disconnect/([^/]+)$', async (request, params) => {
    try {
      const type = params[0];
      ...
      return createErrorResponse('Disconnect error', 500, error);
    }
  }]);

  // GET /api/telemetry/status/:type - Get simulator status
  routes.push(['GET', '^/api/telemetry/status/([^/]+)$', async (request, params) => {
    try {
      const type = params[0];
      ...
      return createErrorResponse('Status check failed', 500, error);
    }
  }]);

  // POST /api/telemetry/stream/start - Start telemetry stream
  routes.push(['POST', '/api/telemetry/stream/start', async (request) => {
    try {
      const body = await request.json();
      ...
      stream.unsubscribe = simulator.subscribe((data) => {
        const telemetryData = {
          ...data,
          receivedAt: Date.now(),
          streamId: stream.id,
      ...
      return createErrorResponse('Failed to start stream', 500, error);
    }
  }]);

  // GET /api/telemetry/stream/:streamId - Get stream data
  routes.push(['GET', '^/api/telemetry/stream/([^/]+)$', async (request, params) => {
    try {
      ...
      if (since > 0) {
        data = data.filter(item => item.receivedAt > since);
      }
      ...
      return createErrorResponse('Failed to get stream data', 500, error);
    }
  }]);

  // DELETE /api/telemetry/stream/:streamId - Stop stream
  routes.push(['DELETE', '^/api/telemetry/stream/([^/]+)$', async (request, params) => {
    try {
      const streamId = params[0];
      ...
      return createErrorResponse('Failed to stop stream', 500, error);
    }
  }]);

  // POST /api/telemetry/command - Send command to simulator
  routes.push(['POST', '/api/telemetry/command', async (request) => {
    try {
      const body = await request.json();
      ...
      return createErrorResponse('Command execution failed', 500, error);
    }
  }]);

  // POST /api/telemetry/commands/batch - Send multiple commands
  routes.push(['POST', '/api/telemetry/commands/batch', async (request) => {
    try {
      const body = await request.json();
      ...
      return createJSONResponse({
        ...
        commands: commandObjects.map(cmd => ({
          id: cmd.id,
          type: cmd.type,
          action: cmd.action,
      ...
      return createErrorResponse('Batch command execution failed', 500, error);
    }
  }]);

  // GET /api/telemetry/commands - Get available commands for simulator
  routes.push(['GET', '^/api/telemetry/commands/([^/]+)$', async (request, params) => {
    try {
      const type = params[0];
      ...
      return createErrorResponse('Failed to get command list', 500, error);
    }
  }]);

  // POST /api/telemetry/distribution/create - Create distribution stream for telemetry
  routes.push(['POST', '/api/telemetry/distribution/create', async (request) => {
    try {
      const body = await request.json();
      const { 
        simulatorType, 
      ...
      return createErrorResponse('Failed to create distribution stream', 500, error);
    }
  }]);

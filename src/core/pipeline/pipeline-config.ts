...
import { createConfigValidator, type ConfigValidator } from '../configuration/config-validator.js';
...
// Type definitions for pipeline configurations
...
export interface AgeEstimationConfig {
  ...
  ageRangeMapping: {
    ...
    teen: readonly [number, number];
    adult: readonly [number, number];
    senior: readonly [number, number];
  };
...
export interface MediapipeFaceMeshConfig {
  maxNumFaces: number;
...
export interface IrisTrackingConfig {
  maxNumFaces: number;
  minDetectionConfidence: number;
  minTrackingConfidence: number;
  refineLandmarks: boolean;
...
export type PipelineConfig = BaseConfig & {
  type: PipelineType;
} & (
  | AgeEstimationConfig
  | EmotionAnalysisConfig
  | MediapipeFaceMeshConfig
  | MediapipeFaceConfig
  | IrisTrackingConfig
);
...
const TYPE_SPECIFIC_CONFIGS: Record<PipelineType, Record<string, unknown>> = {
  'age-estimation': {
    inputSize: [64, 64] as const,
    enableGenderDetection: true,
    ageRangeMapping: {
      child: [0, 12] as const,
      teen: [13, 19] as const,
      adult: [20, 64] as const,
      senior: [65, 100] as const
    },
    modelUrl: null,
    batchSize: 1
  },
  ...
  'emotion-analysis': {
    ...
    batchSize: 1,
    emotionLabels: ['angry', 'disgusted', 'fearful', 'happy', 'sad', 'surprised', 'neutral'] as const,
    enableWebGL: true
  },
...
const VALIDATION_RULES: Record<PipelineType, ValidationRules> = {
  'age-estimation': {
 ...
 */
const _validateConfig = (type: PipelineType, config: Record<string, unknown>): boolean => {
...
export const createPipelineConfig = (
  ...
  const validator: ConfigValidator = createConfigValidator({ _strictMode: false });
  ...
  if (!options.skipSecurityValidation && Object.keys(userConfig).length > 0) {
    ...
    try {
      ...
      handleError(
        `Pipeline configuration validation failed: ${errorMessage}`,
        ErrorCategory.VALIDATION,
        ErrorSeverity.ERROR,
        { type, userConfig }
      );
  ...
  return sanitizedConfig as PipelineConfig;
};

/**
 * Gets default configuration for a pipeline type
 */
export const getDefaultConfig = (type: PipelineType): PipelineConfig => {
 ...
 */
export const areConfigsCompatible = (
  config1: PipelineConfig | null, 
  config2: PipelineConfig | null
): boolean => {
  ...
  const mediaPipeTypes: readonly PipelineType[] = ['mediapipe-face-mesh', 'mediapipe-face', 'iris-tracking'];
 ...
 */
export const getConfigSchema = (type: PipelineType): ConfigSchema | null => {
 ...
 */
export const createInternalPipelineConfig = (
  type: PipelineType, 
...
export const SUPPORTED_PIPELINE_TYPES: readonly PipelineType[] = Object.keys(TYPE_SPECIFIC_CONFIGS) as PipelineType[];

...
export const createMediaPipeFaceDetector = (config = {}) => {
  ...
  // Compatible with TensorFlow.js BlazeFace API
  const estimateFaces = async (imageData, options = {}) => {
    const faces = await detectFaces(imageData, options);
    
    // Convert to TensorFlow.js BlazeFace format for compatibility
    return faces.map(face => ({
      topLeft: [face.boundingBox.x, face.boundingBox.y],
      bottomRight: [
  ...
  return {
    ...
    initialize,
    detectFaces,
    estimateFaces, // BlazeFace compatibility

 ...
 * TypeScript implementation using Bun's native capabilities for WebSocket, HTTP/3, and other protocols
...
export const createWebSocketTransport = (config: WebSocketConfig = {}): WebSocketTransport => {
  ...
  const startServer = (): any => {
      ...
      websocket: {
        open(ws) {
          ...
          state.callbacks.onConnect.forEach(cb => {
            try {
              ...
              logger.warn('Connect callback error:', error);
            }
          });
        },
        ...
        message(ws, message) {
          ...
          try {
            ...
            logger.warn('Message parsing error:', error);
            return;
          }
          ...
          state.callbacks.onMessage.forEach(cb => {
            try {
              ...
              logger.warn('Message callback error:', error);
            }
          });
        },
        ...
        close(ws) {
          ...
          state.callbacks.onDisconnect.forEach(cb => {
            try {
              ...
              logger.warn('Disconnect callback error:', error);
            }
          });
        },
        ...
        error(ws, error) {
          ...
          state.callbacks.onError.forEach(cb => {
            try {
              cb(error instanceof Error ? error : new Error(String(error)), ws);
            } catch (cbError) {
              logger.warn('Error callback failed:', cbError);
            }
          });
        }
      }
    ...
    logger.info(`WebSocket server listening on port ${state.port}`);
    return state.server;
  };
        ...
        state.clientSocket.onclose = () => {
          ...
          if (state.reconnect.enabled && state.reconnect.attempts < state.reconnect.maxAttempts) {
            setTimeout(() => {
              ...
              logger.info(`Reconnect attempt ${state.reconnect.attempts}/${state.reconnect.maxAttempts}`);
              connect(url);
  ...
  const send = (data: MessageData): number => {
    ...
    if (state.isServer) {
      ...
      for (const [id, ws] of state.connections) {
        try {
          ...
          logger.warn(`Failed to send to connection ${id}:`, error);
        }
      }
      return sent;
    } else {
  ...
  const sendTo = (connectionId: string, data: MessageData): boolean => {
    ...
    if (ws) {
      try {
        ...
        logger.warn(`Failed to send to ${connectionId}:`, error);
        return false;
      }
    }
    return false;
  ...
  // Event handlers
  const onConnect = (callback: ConnectCallback): UnsubscribeFunction => {
  ...
  const onMessage = (callback: MessageCallback): UnsubscribeFunction => {
  ...
  const onDisconnect = (callback: DisconnectCallback): UnsubscribeFunction => {
...
export const createHttpTransport = (config: HttpConfig = {}): HttpTransport => {
  ...
  const send = async (endpoint: string, data?: any, options: HttpOptions = {}): Promise<TransportResponse> => {
  ...
  const get = (endpoint: string, options: HttpOptions = {}): Promise<TransportResponse> => 
    send(endpoint, null, { ...options, method: 'GET' });
  
  const post = (endpoint: string, data?: any, options: HttpOptions = {}): Promise<TransportResponse> => 
    send(endpoint, data, { ...options, method: 'POST' });
  
  const put = (endpoint: string, data?: any, options: HttpOptions = {}): Promise<TransportResponse> => 
    send(endpoint, data, { ...options, method: 'PUT' });
  
  const del = (endpoint: string, options: HttpOptions = {}): Promise<TransportResponse> => 
    send(endpoint, null, { ...options, method: 'DELETE' });

  // Eye tracker specific API methods
  const getStatus = (): Promise<TransportResponse> => get('/api/status');
  
  const startRecording = (recordingId: string, config: Partial<RecordingConfig> = {}): Promise<TransportResponse> => 
    post('/api/recordings', { 
      recording_id: recordingId,
      ...config 
    });
  
  const stopRecording = (): Promise<TransportResponse> => post('/api/recordings/stop');
  const getRecordings = (): Promise<TransportResponse> => get('/api/recordings');
  const startCalibration = (): Promise<TransportResponse> => post('/api/calibration/start');
  const stopCalibration = (): Promise<TransportResponse> => post('/api/calibration/stop');
  const getCalibration = (): Promise<TransportResponse> => get('/api/calibration');
  ...
  return {
    ...
    // Configuration
    getBaseUrl: (): string => state.baseUrl,
    setTimeout: (timeout: number): void => { state.timeout = timeout; },
...
export const createUdpTransport = (config: UdpConfig = {}): UdpTransport => {
  const state: UdpState = {
    socket: null,
    port: config.port || 8080,
  ...
  const send = async (data: MessageData): Promise<void> => {
    throw new Error('UDP transport not yet implemented - waiting for Bun UDP support');
  };
...
export const createTransportFactory = (): TransportFactoryRegistry => {
  const transports = new Map<TransportProtocol, TransportFactory>();
  ...
  const register = (protocol: TransportProtocol, factory: TransportFactory): void => {
    transports.set(protocol, factory);
  };

  const create = (protocol: TransportProtocol, config: any = {}): any => {
  ...
  const getAvailableProtocols = (): TransportProtocol[] => Array.from(transports.keys());

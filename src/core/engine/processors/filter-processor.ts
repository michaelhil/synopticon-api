...
// Type definitions
export type FilterType = 'BLUR' | 'GAUSSIAN_BLUR' | 'SHARPEN' | 'EDGE_DETECTION' | 'EMBOSS';

export interface FilterProcessor {
  applyFilter: (imageData: ImageData, filterType: string, intensity?: number) => ImageData;
  applyConvolution: (imageData: ImageData, kernel: number[][], factor?: number, bias?: number) => ImageData;
  gaussianBlur: (imageData: ImageData, radius?: number) => ImageData;
  medianFilter: (imageData: ImageData, radius?: number) => ImageData;
  unsharpMask: (imageData: ImageData, amount?: number, radius?: number, threshold?: number) => ImageData;
  bilateralFilter: (imageData: ImageData, spatialSigma?: number, colorSigma?: number) => ImageData;
  getAvailableFilters: () => string[];
  getKernel: (filterType: string) => number[][] | undefined;
}

export const createFilterProcessor = (_resourcePool: any): FilterProcessor => {
   ...
   */
  const applyConvolution = (imageData: ImageData, kernel: number[][], factor: number = 1, bias: number = 0): ImageData => {
    try {
      ...
      handleError(
        `Convolution filter failed: ${(error as Error).message}`,
   ...
   */
  const gaussianBlur = (imageData: ImageData, radius: number = 1): ImageData => {
    try {
      ...
      handleError(
        `Gaussian blur failed: ${(error as Error).message}`,
        ErrorCategory.PROCESSING,
        ErrorSeverity.ERROR,
        { radius }
      );
   ...
   */
  const applyFilter = (imageData: ImageData, filterType: string, intensity: number = 1.0): ImageData => {
    try {
      ...
      handleError(
        `Filter application failed: ${(error as Error).message}`,
        ErrorCategory.PROCESSING,
        ErrorSeverity.ERROR,
   ...
   */
  const medianFilter = (imageData: ImageData, radius: number = 1): ImageData => {
    try {
      const { data, width, height } = imageData;
      const outputData = new Uint8ClampedArray(data.length);
      ...
      handleError(
        `Median filter failed: ${(error as Error).message}`,
        ErrorCategory.PROCESSING,
        ErrorSeverity.ERROR,
        { radius }
      );
   ...
   */
  const unsharpMask = (imageData: ImageData, amount: number = 1.0, radius: number = 1.0, threshold: number = 0): ImageData => {
    try {
      ...
      handleError(
        `Unsharp mask failed: ${(error as Error).message}`,
        ErrorCategory.PROCESSING,
        ErrorSeverity.ERROR,
   ...
   */
  const bilateralFilter = (imageData: ImageData, spatialSigma: number = 5, colorSigma: number = 50): ImageData => {
    try {
      const { data, width, height } = imageData;
      const outputData = new Uint8ClampedArray(data.length);
      ...
      handleError(
        `Bilateral filter failed: ${(error as Error).message}`,
        ErrorCategory.PROCESSING,
        ErrorSeverity.ERROR,
  ...
  return {
    ...
    getAvailableFilters: (): string[] => Object.keys(KERNELS),
    
    // Export kernels for advanced use
    getKernel: (filterType: string): number[][] | undefined => KERNELS[filterType.toUpperCase()]
  };
};